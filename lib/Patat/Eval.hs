...
--------------------------------------------------------------------------------
eval :: EvalSettings -> Pandoc -> IO Pandoc
eval settings = Pandoc.walkM $ fmap concat . mapM (evalBlock settings)


--------------------------------------------------------------------------------
evalBlock :: EvalSettings -> Pandoc.Block -> IO [Pandoc.Block]
evalBlock s@EvalSettings {..} (Pandoc.CodeBlock attr@(_, classes, _) txt)
    | "eval" `elem` classes = unsafeInterleaveIO $ do
        ...
        let out = case erExitCode of
                ...
                ExitFailure i ->
                    ...
                    erStderr
        pure [Pandoc.CodeBlock attr out]
evalBlock _ block =
    pure [block]
